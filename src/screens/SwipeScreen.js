...
export function SwipeScreen() {
      ...
      try {
        ...
        if (status === 'granted') {
          ...
          if (media.assets.length > 0) {
            console.log('First asset type:', media.assets[0].mediaType);
          }
  ...
  const undoLastAction = () => {
    if (actionHistory.length > 0) {
      ...
      if (lastAction.action === 'delete') {
        setToDeleteAssets(prev => prev.filter(asset => asset.id !== lastAction.asset.id));
      }
  ...
  if (!permission) {
    return (
      <View style={styles.container}>
        <Text style={styles.message}>Please grant media library permissions to use this feature.</Text>
      </View>
    );
  }
  ...
  if (mediaAssets.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.message}>No media found in your gallery.</Text>
      </View>
    );
  }
  ...
  return (
    <GestureHandlerRootView style={styles.container}>
      <View style={styles.container}>
          ...
          {actionHistory.length > 0 && (
            ...
            >
              ...
              <Text style={styles.undoButtonText}>
                Undo {actionHistory[actionHistory.length - 1]?.action}
              </Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <TouchableOpacity
          style={[styles.proceedButton, { opacity: toDeleteAssets.length > 0 ? 1 : 0.5 }]}
